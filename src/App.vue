<template>
  <div id="app" class="container">
    <header>
      <h1>The Billion Dollar NFT</h1>
      <h2>
        100,000,000 pixels &middot; 0.001 ETH per pixel &middot;BE APART OF THE
        GLOBAL NFT COLLAGE, BE APART OF HISTORY!
      </h2>
      <div class="sold" v-if="ready">
        {{ $store.state.adsPixels }} pixels sold
        <button
          v-on:click="$store.commit('updatePreview', { x: 20, y: 20 })"
          v-if="!$store.state.previewAd"
        >
          Buy Pixels
        </button>
      </div>
    ...
    <div class="sold" v-if="ready">
      {{ $store.state.adsPixels }} pixels sold
      <button
        v-on:click="$store.commit('updatePreview', { x: 20, y: 920 })"
        v-if="!$store.state.previewAd"
      >
        Buy Pixels
      </button>
    ...
    <div class="info">
      <p>
        ...
        <a href="https://github.com/ethereum/web3.js/">Web3.js</a>
        . Pretty cool, right?
      </p>
      <p>
    ...
    <footer>
      <ul>
        <li><h3>Authors</h3></li>
        <li><a href="https://hireelisha.com">Elisha Day</a></li>
      </ul>
      <ul>
        <li><h3>Project</h3></li>
...
export default {
  ...
  methods: {
    setNetwork(network) {
      ...
      this.ready = false
      waitForWeb3(
        deployConfig[network || defaultNetwork],
        function (web3) {
          ...
          this.web3.version.getNetwork(
            function (error, networkVersion) {
              ...
              this.isReadOnly =
                providerHost && providerHost.indexOf('infura.io') !== -1
              if (this.activeNetwork === undefined) {
              ...
              if (web3.currentProvider.isMetaMask) {
                ...
                const interval = setInterval(function () {
                  web3.version.getNetwork(
                    function (error, newNetworkVersion) {
